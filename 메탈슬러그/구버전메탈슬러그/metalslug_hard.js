function gameHard(){
    ...
    function moveBoss(enemy) {
        setInterval(() => {
            ...
            const newLeft = currentLeft + bossDirection * 500; // 50px 이동
        ...
        if (level < 2) {
            ...
            if (!bossCreated) {
                ...
                if (random === 1) {
                    ...
                    enemy.style.left = `${Math.max(0, Math.min((canvas.width - brickWidth) / 2, brickX))}px`;
                    enemy.style.top = `680px`;
                    enemy.dataset.direction = 'right'; // 초기 방향 설정 **
                } else {
            ...
            if (bricks[c] && bricks[c][r]) { // 존재하는 벽돌인지 체크
                ...
                if (!enemy.src.includes('boss2.gif')) {
                    if (enemy.src.includes('enemy3.gif')) { // enemy3.gif의 경우
                        //enemy.style.left = `${currentLeft + 4 * speed}px`; // 오른쪽으로 이동 기존 

                        let direction = enemy.dataset.direction;

                        if (direction === 'right') {
                            enemy.style.left = `${enemyLeft + 4 * speed}px`;
                            if (enemyLeft + 4 * speed >= canvas.width - brickWidth) {
                                enemy.dataset.direction = 'left';
                            }
                        } else if (direction === 'left') {
                            enemy.style.left = `${enemyLeft - 4 * speed}px`;
                            if (enemyLeft - 4 * speed <= 0) {
